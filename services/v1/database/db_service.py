...
_old_db_pool: Optional[ThreadedConnectionPool] = None

def get_old_pool() -> ThreadedConnectionPool:
    """Lazy initialization of the OLD database connection pool for reading libraries."""
    global _old_db_pool
    if _old_db_pool is None:
        try:
            _old_db_pool = ThreadedConnectionPool(
                minconn=1,
                maxconn=5,
                host=os.getenv("OLD_DB_HOST", os.getenv("DB_HOST", "localhost")),
                database=os.getenv("OLD_DB_NAME", "postgres"),
                user=os.getenv("OLD_DB_USER", "postgres"),
                password=os.getenv("OLD_DB_PASS", ""),
                port=os.getenv("OLD_DB_PORT", "5432")
            )
            logger.info("OLD Database connection pool initialized")
        except Exception as e:
            logger.error(f"Failed to initialize OLD database pool: {e}")
            raise
    return _old_db_pool
...
class OldDBConnection:
    """Context manager for OLD PostgreSQL database connections (read-only for libraries)."""
    def __init__(self, commit: bool = False, use_dict_cursor: bool = False):
        self.commit = commit
        self.use_dict_cursor = use_dict_cursor
        self.pool = get_old_pool()
        self.conn = None
        self.cursor = None
    ...
    def __enter__(self):
        try:
            ...
            logger.error(f"OLD Database connection error: {e}")
            if self.conn:
                self.pool.putconn(self.conn)
            raise
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            if exc_type:
                logger.error(f"OLD DB Transaction rollback due to: {exc_val}")
                self.conn.rollback()
            elif self.commit:
                self.conn.commit()
        finally:
...
def init_db() -> None:
    ...
    tables = [
        """CREATE TABLE IF NOT EXISTS clients (
            ...
            subtitle_mode TEXT DEFAULT 'phrase_block',
            subtitle_style_preset TEXT DEFAULT 'impact',
            subtitle_font_family TEXT DEFAULT 'montserrat',
            ...
            subtitle_outline_width NUMERIC(4,1) DEFAULT 1.5,
            subtitle_margin_v INTEGER DEFAULT 520,
            subtitle_margin_percent INTEGER DEFAULT 27,
            deepgram_keywords TEXT,
            deepgram_vocabulary_rules JSONB DEFAULT '[]'::jsonb,
        ...
        """CREATE TABLE IF NOT EXISTS client_products (
            ...
            product_media_assets JSONB DEFAULT '[]'::jsonb,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )""",
        ...
        """CREATE TABLE IF NOT EXISTS generated_scenarios (
            ...
            montage_title TEXT, montage_description TEXT,
        ...
        """CREATE TABLE IF NOT EXISTS client_omni_avatars (
            id SERIAL PRIMARY KEY,
            client_id INTEGER REFERENCES clients(id) ON DELETE CASCADE,
            avatar_name TEXT NOT NULL,
            prompt TEXT,
        ...
        # Migration helpers
        "ALTER TABLE topic_cards ADD COLUMN IF NOT EXISTS library_id INTEGER",
        "ALTER TABLE structure_cards ADD COLUMN IF NOT EXISTS library_id INTEGER",
        "ALTER TABLE topic_structure_pairs ADD COLUMN IF NOT EXISTS library_id INTEGER",
        "ALTER TABLE clients ADD COLUMN IF NOT EXISTS active_product_id INTEGER REFERENCES client_products(id)",
...
def get_processed_content_by_job_id(job_id: str) -> Optional[Dict[str, Any]]:
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("SELECT * FROM processed_content WHERE job_id = %s", (job_id,))
        row = cursor.fetchone()
        return dict(row) if row else None
...
def get_random_entity(table: str, client_id: int, exclude_ids: Optional[Set[int]] = None) -> Optional[Dict[str, Any]]:
    with DBConnection(use_dict_cursor=True) as cursor:
        where_clause = "(client_id = %s OR library_id IN (SELECT library_id FROM client_libraries WHERE client_id = %s))"
        ...
        cursor.execute(f"SELECT * FROM {table} WHERE {where_clause} ORDER BY RANDOM() LIMIT 1", params)
        row = cursor.fetchone()
        return dict(row) if row else None
...
def get_random_topic_family_card(client_id: int, exclude_ids: Optional[Set[int]] = None) -> Optional[Dict[str, Any]]:
    with DBConnection(use_dict_cursor=True) as cursor:
        ...
        cursor.execute(f"""
            ...
            WHERE {where_clause} ORDER BY canonical_topic_family, created_at DESC
        """, params)
...
def get_random_compatible_pair(client_id: int, exclude_pairs: Optional[Set[Tuple[int, int]]] = None) -> Optional[Dict[str, Any]]:
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("""
            ...
            WHERE (p.client_id = %s OR p.library_id IN (SELECT library_id FROM client_libraries WHERE client_id = %s)) 
...
def get_random_compatible_family_pair(client_id: int, exclude_pairs: Optional[Set[Tuple[int, int]]] = None) -> Optional[Dict[str, Any]]:
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("""
            ...
            JOIN structure_cards s ON s.id = p.structure_card_id
            WHERE (p.client_id = %s OR p.library_id IN (SELECT library_id FROM client_libraries WHERE client_id = %s))
            ORDER BY t.canonical_topic_family, s.canonical_pattern_key, p.pair_count DESC, p.updated_at DESC
        """, (client_id, client_id))
        rows = cursor.fetchall()
        if not rows: return None
        if exclude_pairs:
...
def create_client_product(client_id: int, name: str, product_info: str) -> int:
    """Creates a new product for a client."""
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("""
            INSERT INTO client_products (client_id, name, product_info)
            VALUES (%s, %s, %s)
            RETURNING id
        """, (client_id, name, product_info))
        row = cursor.fetchone()
        return row["id"]

def get_client_products(client_id: int) -> List[Dict[str, Any]]:
    """Gets all products for a client."""
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("""
            SELECT * FROM client_products WHERE client_id = %s ORDER BY created_at ASC
        """, (client_id,))
        return [dict(r) for r in cursor.fetchall()]

def set_active_product(client_id: int, product_id: int) -> None:
    """Sets the active product for a client."""
    with DBConnection() as cursor:
        cursor.execute("""
            UPDATE clients SET active_product_id = %s WHERE id = %s
        """, (product_id, client_id))

# --- Content Library Management Functions ---

def create_library(name: str, description: str = "") -> int:
...
def get_all_libraries() -> List[Dict[str, Any]]:
    """Gets all content libraries available on the server."""
    with OldDBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("SELECT * FROM content_libraries ORDER BY created_at ASC")
        return [dict(r) for r in cursor.fetchall()]

def get_client_libraries(client_id: int) -> List[Dict[str, Any]]:
    """Gets all libraries linked to a specific client."""
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("SELECT library_id FROM client_libraries WHERE client_id = %s", (client_id,))
        rows = cursor.fetchall()
        if not rows:
            return []
    ...
    with OldDBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("SELECT id, name, description FROM content_libraries WHERE id = ANY(%s)", (lib_ids,))
        return [dict(r) for r in cursor.fetchall()]

def link_library_to_client(client_id: int, library_id: int) -> None:
    """Links a content library to a client (project)."""
    with DBConnection() as cursor:
        cursor.execute("""
...
def unlink_library_from_client(client_id: int, library_id: int) -> None:
    """Unlinks a content library from a client (project)."""
    with DBConnection() as cursor:
        cursor.execute("""
            DELETE FROM client_libraries
            WHERE client_id = %s AND library_id = %s
        """, (client_id, library_id))

def get_active_product(client_id: int) -> Optional[Dict[str, Any]]:
    """Gets the currently active product for a client. If none is explicitly active, returns the first one."""
    with DBConnection(use_dict_cursor=True) as cursor:
        # Check explicit active product
        cursor.execute("""
            SELECT p.* FROM client_products p
            JOIN clients c ON c.active_product_id = p.id
            WHERE c.id = %s
        """, (client_id,))
        ...
        # Fallback to the first product created
        cursor.execute("""
...
def get_active_omni_avatar_by_client_id(client_id: int) -> Optional[Dict[str, Any]]:
    """Retrieve the active Omni avatar for a client."""
    with DBConnection(use_dict_cursor=True) as cursor:
        cursor.execute("""
            SELECT * FROM client_omni_avatars
            WHERE client_id = %s AND is_active = TRUE AND status = 'approved'
            ORDER BY created_at DESC
            LIMIT 1
        """, (client_id,))
        row = cursor.fetchone()
...
def _cancel_pending_web_auth_requests_for_user(
    cursor: Any,
...
def request_telegram_access(
    ...
    with DBConnection(use_dict_cursor=True) as cursor:
        ...
        if str(payload.get("status") or "").lower() == "approved":
            _cancel_pending_web_auth_requests_for_user(cursor, int(telegram_user_id))
        return payload
