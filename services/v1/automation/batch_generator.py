...
def _resolve_character_targets_for_duration(
    ...
    fallback_duration = _safe_float(target_duration_seconds, 0.0) or 50.0
    resolved_min = _safe_float(target_duration_min_seconds, fallback_duration)
    resolved_max = _safe_float(target_duration_max_seconds, resolved_min)
    resolved_min = max(1.0, resolved_min)
...
def _rewrite_script_for_duration_overflow(
    ...
    *,
    ...
    if not (resolved_target_min and resolved_target_max and resolved_target_max >= resolved_target_min):
        source_chars = count_spoken_characters(source_script)
...
def _resolve_avatar_generation_context(
...
) -> tuple[dict | None, str, str | None, str | None, str, float | None]:
    ...
    if active_avatar:
        resolved_cpm = _safe_float(active_avatar.get("tts_chars_per_minute"), 0.0)
        if resolved_cpm > 0:
            selected_voice_chars_per_minute = resolved_cpm
...
def run_batch_generation(count=1, client_id=1, niche="General", topic=None, angle=None, mode="rewrite", topic_id=None, structure_id=None, generation_source="manual"):
    ...
    from services.v1.database.db_service import get_active_product
    active_product = get_active_product(client_id)
    target_product_info = active_product.get("product_info") if active_product else client.get("product_info")
    ...
    for i in range(count):
        ...
        active_avatar, selected_tts_provider, selected_tts_voice_id, selected_elevenlabs_voice_id, gender, selected_voice_chars_per_minute = _resolve_avatar_generation_context(
            client_id=client_id,
            ...
            try:
                ...
                for rewrite_attempt in range(SCENARIO_DURATION_REWRITE_MAX_ATTEMPTS + 1):
                    ...
                    if is_overflow_after_fit and rewrite_attempt < SCENARIO_DURATION_REWRITE_MAX_ATTEMPTS:
                        ...
                        if rewritten_script:
                            logger.warning(
                                "Scenario %s over max duration after TTS fit (%.2fs > %.2fs). Rewriting script for retry %s/%s.",
                                res_job_id,
                            ...
                            scenario["script"] = rewritten_script
                            continue
                    ...
                    if is_overflow_after_sentence_trim and rewrite_attempt < SCENARIO_DURATION_REWRITE_MAX_ATTEMPTS:
                        ...
                        if rewritten_script:
                            logger.warning(
                                "Scenario %s still over max duration after sentence trim (%.2fs > %.2fs). Rewriting script for retry %s/%s.",
                                res_job_id,
                                float(tts_audio_duration_seconds or 0.0),
                                duration_overflow_limit_seconds,
                                rewrite_attempt + 1,
                    ...
                    if is_overflow_after_sentence_trim:
                        logger.warning(
                            "Scenario %s remains above configured max+tolerance (%.2fs > %.2fs) after all retries.",
                            res_job_id,
                            float(tts_audio_duration_seconds or 0.0),
                            duration_overflow_limit_seconds,
                        )
