...
from services.v1.database.db_service import (
    ...
    get_active_omni_avatar_by_client_id,
    update_generated_scenario,
)
...
def process_scenario_stage(job: Dict[str, Any]) -> None:
    ...
    # IMPORTANT: start with Omni generation.
    next_stage = "omni_generate"
...
def process_omni_generate_stage(job: Dict[str, Any]) -> None:
    ...
    # 1. Fetch approved avatar and clothing
    avatar = None
    if client_id:
        avatar = get_active_omni_avatar_by_client_id(int(client_id))
    
    # Fallbacks if client has no avatar
    clothing = "повседневной одежде"
    character_id = "2eefe60357c743d3911f3f980fe8304a"  # default
    audio_id = "4a786461922c4383a2010d9b8a4b4f33" # default
    ...
    script = _extract_scenario_script(scenario)
    if not script:
        raise RuntimeError("Scenario has no script to generate video for.")

    # 3. Apply Omni Smart Layer
