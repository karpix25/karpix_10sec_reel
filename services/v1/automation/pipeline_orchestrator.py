...
def run_content_gen_pipeline(job_id, transcript=None, reels_url=None, niche="General", target_product_info=None, client_id=None, avatar_id="469083313936440c9d9651586bd2251a", analysis_only=False, generate_video=True, manual=True, product_id=None):
    """
    ...
    if client_id:
        ...
        if client_data:
            ...
            brand_voice = brand_voice or client_data.get("brand_voice")
            target_audience = target_audience or client_data.get("target_audience")
            
            # Fetch product
            if not target_product_info:
                active_product = get_active_product(client_id)
                if active_product:
                    target_product_info = active_product.get("product_info")
                    logger.info(f"Using product info from DB for client {client_id}: {active_product.get('name')}")
