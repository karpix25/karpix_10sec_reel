...
def build_connection_help_message(message, config=None):
    ...
    lines = [
        "Как запустить проект и сохранить видео в нужную папку",
        "",
        "1. На Яндекс.Диске вручную создайте папку проекта / бренда.",
        "Например: ВИДЕО/АВТОМАТ/Molekular",
        "",
        "2. В Telegram создайте проект:",
        "/new_client Molekular",
        "",
        "3. Перейдите в нужный топик и привяжите проект к этому топику:",
        "/assign_client Molekular",
        "",
        "4. В веб-интерфейсе откройте настройки проекта и выберите созданную папку Яндекс.Диска.",
        "Эта папка станет основной папкой проекта.",
        "",
        "5. При необходимости задайте контекст проекта:",
        "/set_client_product ...",
        "/set_brand ...",
        "",
        "6. После этого запускайте генерацию или отправляйте Instagram Reel ссылку в этот топик.",
        "",
        "Куда сохранятся финальные видео:",
        "выбранная папка проекта / имя аватара / финальное видео.mp4",
        "",
        "Папки аватаров внутри выбранной папки создаются автоматически при генерации нового финального видео.",
        "",
...
def handle_set_client_product(message):
    ...
    try:
        from services.v1.database.db_service import create_client_product, set_active_product
        # We need a name for the product, we'll take the first 20 chars
        product_name = f"Продукт: {product[:20]}..."
        product_id = create_client_product(config["client_id"], product_name, product)
        set_active_product(config["client_id"], product_id)
        
        _reply_in_same_thread(message, f"✅ Создан новый продукт и установлен как активный:\n\n{product}")
    except Exception as e:
        _reply_in_same_thread(message, f"❌ Ошибка при создании продукта: {e}")
...
def handle_list_products(message):
    ...
    try:
        from services.v1.database.db_service import get_client_products, get_active_product
        products = get_client_products(config["client_id"])
        active = get_active_product(config["client_id"])
        active_id = active["id"] if active else None
        
        if not products:
            _reply_in_same_thread(message, "У этого проекта пока нет продуктов. Создайте первый: /set_client_product [описание]")
            return
            
        msg = "📦 <b>Продукты проекта:</b>\n\n"
        for p in products:
            icon = "✅" if p["id"] == active_id else "➖"
            msg += f"{icon} <b>ID {p['id']}</b>: {p['name']}\n<i>{p['product_info'][:50]}...</i>\n/activate_product_{p['id']}\n\n"
            
        bot.send_message(message.chat.id, msg, parse_mode='HTML', message_thread_id=message.message_thread_id)
    except Exception as e:
        logger.error(f"Error listing products: {e}")
        _reply_in_same_thread(message, "❌ Ошибка при получении списка продуктов")

@bot.message_handler(regexp=r'^/activate_product_(\d+)$')
def handle_activate_product(message):
    if not _authorize_message(message):
        return
    ...
    import re
    match = re.match(r'^/activate_product_(\d+)$', message.text.strip())
    if match:
        product_id = int(match.group(1))
        try:
            from services.v1.database.db_service import set_active_product
            set_active_product(config["client_id"], product_id)
            _reply_in_same_thread(message, f"✅ Активный продукт изменен на ID {product_id}")
        except Exception as e:
            _reply_in_same_thread(message, f"❌ Ошибка при активации продукта: {e}")

@bot.message_handler(commands=['libraries'])
def handle_list_libraries(message):
    if not _authorize_message(message): return
    topic_id = str(message.message_thread_id or "0")
    config = get_topic_config(topic_id)
    if not config:
        _reply_in_same_thread(message, "Сначала выберите проект: /clients")
        return
        
    try:
        from services.v1.database.db_service import get_all_libraries, get_client_libraries
        all_libs = get_all_libraries()
        linked_libs = [lib["id"] for lib in get_client_libraries(config["client_id"])]
        
        if not all_libs:
            _reply_in_same_thread(message, "На сервере пока нет общих библиотек контента.")
            return
            
        msg = "📚 <b>Доступные библиотеки:</b>\n\n"
        for lib in all_libs:
            icon = "✅ Подключена" if lib["id"] in linked_libs else "❌ Не подключена"
            action = f"/unlink_library_{lib['id']}" if lib["id"] in linked_libs else f"/link_library_{lib['id']}"
            
            msg += f"<b>ID {lib['id']}</b>: {lib['name']}\n"
            if lib.get('description'):
                msg += f"<i>{lib['description']}</i>\n"
            msg += f"Статус: {icon}\nДействие: {action}\n\n"
            
        bot.send_message(message.chat.id, msg, parse_mode='HTML', message_thread_id=message.message_thread_id)
    except Exception as e:
        logger.error(f"Error listing libraries: {e}")
        _reply_in_same_thread(message, "❌ Ошибка при получении списка библиотек")

@bot.message_handler(regexp=r'^/(link|unlink)_library_(\d+)$')
def handle_link_unlink_library(message):
    if not _authorize_message(message): return
    topic_id = str(message.message_thread_id or "0")
    config = get_topic_config(topic_id)
    ...
    import re
    match = re.match(r'^/(link|unlink)_library_(\d+)$', message.text.strip())
    if match:
        action = match.group(1)
        library_id = int(match.group(2))
        try:
            from services.v1.database.db_service import link_library_to_client, unlink_library_from_client
            if action == "link":
                link_library_to_client(config["client_id"], library_id)
                _reply_in_same_thread(message, f"✅ Библиотека ID {library_id} успешно подключена к проекту!")
            else:
                unlink_library_from_client(config["client_id"], library_id)
                _reply_in_same_thread(message, f"❌ Библиотека ID {library_id} отключена от проекта.")
        except Exception as e:
            _reply_in_same_thread(message, f"❌ Ошибка: {e}")
