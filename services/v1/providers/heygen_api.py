...
def _heygen_request(method: str, url: str, **kwargs: Any) -> Dict[str, Any]:
    ...
    if not response.ok:
        raise HeyGenApiError(
            ...
            payload,
            f"HeyGen request failed with status {response.status_code}",
        )
...
def generate_avatar_video(
    ...
    logger.info(
        "Triggering HeyGen video generation with avatar_id=%s look_id=%s fallback_look_id=%s resolved_look_id=%s",
        avatar_id,
        look_id,
        fallback_look_id,
        resolved_look_id,
