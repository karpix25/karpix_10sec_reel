...
export const STORYBOARD_PLAN_REVIEW_SYSTEM_PROMPT = [
  ...
  "Проверяй одежду только по wardrobe_continuity и wardrobe_timeline режиссерского анализа: stable требует непрерывности, changes_between_cuts допускает одежду соответствующего интервала, not_visible не проверяется, unknown не является основанием для блокировки.",
  ...
  "Не блокируй обычную смену локации или камеры, если она согласуется с reference timeline. Смену одежды оценивай только по wardrobe_continuity и wardrobe_timeline, а не по формату.",
