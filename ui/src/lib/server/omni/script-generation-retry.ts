...
export function buildScriptRetryFeedback(
  ...
  if (/должно делиться на четыре|нельзя разложить по кадрам|остаток только в одно/iu.test(message)) {
    ...
    return [
      ...
      `Перепиши его с тем же смыслом и естественной подачей на ${nextValidWords || "подходящее"} слов: количество должно делиться на четыре или иметь остаток одно-три слова.`,
      "Ничего не обрезай: если не хватает слов, добавь конкретный пример или короткое уточнение по смыслу, а не пустой повтор.",
      "Каждый двухсекундный кадр обычно содержит четыре слова, а одно-три лишних слова можно распределить по последним кадрам последней части. beats.voiceover и script должны иметь одинаковое количество слов.",
    ].join(" ");
  }
