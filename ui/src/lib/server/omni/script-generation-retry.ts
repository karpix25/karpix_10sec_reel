...
export function buildScriptRetryFeedback(
  ...
  if (/Не удалось разделить сценарий|диапазон.*секунд|длительности.*секунд/iu.test(message)) {
    return [
      ...
      "Не подгоняй текст механически под количество слов: сохрани хук, факты, пользу продукта и CTA, а длительность рассчитай от естественной речи.",
      "Планировщик сам разобьет текст на части по 4, 6, 8 или 10 секунд, округляя длительность вверх. Каждая граница должна быть только после полного предложения; не оставляй отдельные фрагменты из одного-двух слов и не разрывай союз, предлог или зависимую фразу.",
  ...
  if (/должно делиться на четыре|нельзя разложить по кадрам|остаток только в одно|завершенными предложениями/iu.test(message)) {
    const wordMatch = message.match(/(\d+) слов/iu);
    const currentWords = wordMatch ? Number.parseInt(wordMatch[1] || "0", 10) : 0;
    return [
      `Предыдущий сценарий нельзя разложить по кадрам: в нем ${currentWords || "неподходящее для кадров количество"} слов.`,
      "Перепиши его с тем же смыслом и естественной подачей так, чтобы соседние законченные предложения можно было объединить в части 4, 6, 8 или 10 секунд.",
      "Не подгоняй количество слов под кратность четырем, не добавляй пустой повтор и не разрывай предложение. Не оставляй отдельными частями фразы из одного-двух слов.",
      "Ориентир речи четыре слова на две секунды, но три слова в отдельном кадре допустимы, если этого требует граница предложения. beats.voiceover и script должны иметь одинаковый текст.",
    ].join(" ");
  }
