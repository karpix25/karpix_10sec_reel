...
export function buildScriptRetryFeedback(error: unknown) {
  ...
  if (/доступные Omni-длительности|Максимум .* слов|не помещается в доступные/iu.test(message)) {
    ...
    return [
      `Предыдущий сценарий слишком длинный для лимита из трех или четырех частей${currentWords ? `: ${currentWords} слов` : ""}.`,
      "В этот раз верни не больше девяноста шести слов, сохранив хук, смысл продукта, главный аргумент, конкретный механизм или пример и CTA.",
      "Удали второстепенные пояснения и повторы, но не выбрасывай CTA и не добавляй новые рекламные обещания.",
      "Не переписывай reference с нуля. Сохрани его хук, порядок мыслей, механизм, пример и финал; сокращай только повторы и лишние слова. Верни три или четыре смысловых бита. Поле script и сумма beats.voiceover должны совпадать дословно.",
    ].join(" ");
  }
