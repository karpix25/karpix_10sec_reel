...
export function buildScriptRetryFeedback(
  ...
  if (/должно делиться на четыре|нельзя разложить по кадрам/iu.test(message)) {
    ...
    return [
      ...
      `Перепиши его с тем же смыслом и естественной подачей на ${nextValidWords || "кратное четырем"} слов или другое количество, кратное четырем.`,
      "Ничего не обрезай: если не хватает слов, добавь конкретный пример или короткое уточнение по смыслу, а не пустой повтор.",
      "Каждый двухсекундный кадр должен содержать ровно четыре слова, поэтому beats.voiceover и script должны иметь одинаковое количество слов, кратное четырем.",
    ].join(" ");
  }
