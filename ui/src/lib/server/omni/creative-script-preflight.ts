...
export const CREATIVE_SPEECH_PACKING_RULE = [
  ...
  `Отдельное предложение не длиннее ${getOmniSegmentWordBudget()} слов, включая полное название продукта и числа, записанные словами. Короткий CTA объединяй с соседним законченным предложением только если группа остается в пределах шести-двадцати слов. Если после фразы на семнадцать-двадцать слов остается хвост из одного-пяти слов, раздели или сократи предыдущую фразу на две законченные, а не приклеивай хвост к ней.`,
