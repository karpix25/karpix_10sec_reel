...
export function buildCreativeCopywriterAttemptPrompt(input: {
  ...
  return {
    ...
    prompt: `${basePrompt}\n\nПовторная попытка:\n${buildCreativeCopywriterRebuildFeedback({
      semanticReview: input.semanticReview,
      failureReason: input.failureReason,
      adaptationPlan: input.chainInput.adaptationPlan,
    })}`,
  };
}
...
export function buildCreativeCopywriterRepairPrompt(input: CreativeRepairInput) {
  ...
  const adaptationPlan = chainInput.adaptationPlan;
  return [
    ...
    renderScriptAdaptationRepairContract(adaptationPlan),
    ...
    adaptationPlan.mode === "format_transfer"
      ? "В финальном выводе заверши новый продуктовый тезис. Не возвращай несовместимый предметный вопрос reference."
...
export function buildCreativeCopywriterRebuildFeedback(input: {
  ...
  return [
    ...
    input.adaptationPlan
      ...
      : "Ответь на хук и сохрани контракт reference, встрои продукт через причинный переход к потребности текущего сюжета и поставь CTA после его пользы, до полноценного финального вывода.",
  ].join(" ");
}
