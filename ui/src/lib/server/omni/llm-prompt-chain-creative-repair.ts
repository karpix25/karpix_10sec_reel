...
export function buildCreativeCopywriterRebuildFeedback(input: {
  ...
  return [
    ...
    "Ответь на хук, сохрани основной смысл и конкретные примеры reference, встрои продукт через причинный переход к потребности текущего сюжета и поставь CTA после его пользы, до полноценного финального вывода. В выводе назови главный объект reference и утверди его тезис; вывод не является вопросом, приказом или призывом и не сводится к общей фразе о продукте.",
  ].join(" ");
}
