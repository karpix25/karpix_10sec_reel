...
export function buildCreativeCopywriterRebuildFeedback(input: {
  ...
  return [
    ...
    "Ответь на хук, сохрани основной смысл и конкретные примеры reference, нативно свяжи продукт с конкретным действием, поставь CTA перед полноценным финальным выводом, который является утвердительным и не является вопросом, приказом или призывом.",
  ].join(" ");
}
