...
export function buildCreativeCopywriterRepairPrompt(input: CreativeRepairInput) {
  ...
  return [
    ...
    "Встрой продукт через конкретную потребность или действие, уже присутствующее в истории. Продукт не должен заменять тему reference или выглядеть рекламной паузой.",
    ...
    "Если reference перечисляет конкретные места, действия, услуги или доказательства, сохрани минимум два конкретных примера и цену-payoff; не заменяй их общими словами.",
    "Два отдельных элемента списка считаются двумя примерами, даже если стоят в одном предложении через «и». Не раздувай сценарий дополнительными примерами, если этот минимум уже выполнен.",
...
function renderFailedChecks(review: ScriptSemanticReview | null) {
  ...
  const checks = [
    ...
    [review.productNaturallyIntegrated, "нативная интеграция продукта"],
