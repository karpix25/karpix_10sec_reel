...
export function renderScriptAdaptationContract(plan: ScriptAdaptationPlan): string {
  ...
  return [
    ...
    plan.mode === "writer_owned"
      ? "Сценарист сам строит продуктовую адаптацию, но сохраняет смысловое ядро reference. Обязательны конкретные опоры, которые делают историю узнаваемой: названная география, центральное сравнение или конфликт, ключевые доказательства и тип финального вывода. Не обобщай конкретное место или категорию до расплывчатых слов вроде «у моря», «за границей» или «где-то», если продукт не требует такой замены."
...
export function renderScriptAdaptationReviewContract(plan: ScriptAdaptationPlan): string {
  ...
  if (plan.mode === "writer_owned") {
    return "Сценарист сам выбрал продуктовую адаптацию, но не вправе терять идентичность reference. Проверь названную географию, центральное сравнение или конфликт, ключевые доказательства и тип финального вывода. Более широкое обобщение не равно сохранению конкретной опоры. Связь продукта с мыслью должна быть причинной и подтверждённой входными данными."
  }
