...
export function renderScriptAdaptationContract(plan: ScriptAdaptationPlan): string {
  ...
  return [
    ...
    plan.mode === "writer_owned"
      ? "Сценарист сам определяет смысловое ядро reference и сохраняет его при продуктовой адаптации: тему, обещание хука, логику раскрытия и фрейм подачи. Не подменяй их внешне похожей, но другой историей."
...
export function renderScriptAdaptationReviewContract(plan: ScriptAdaptationPlan): string {
  ...
  if (plan.mode === "writer_owned") {
    return "Проверь, что продуктовая адаптация сохраняет самостоятельно определённые моделью тему, обещание хука, логику раскрытия и фрейм подачи reference. Связь продукта с текущей мыслью должна быть причинной и подтверждённой входными данными."
  }
