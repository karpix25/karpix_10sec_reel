...
export function buildStoryboardImagePrompt(input: {
  ...
  return [
    ...
    "Смысл текущей реплики и соответствующий reference-кадр определяют содержание панели. Переноси из reference только уместный визуальный приём: жест, реакцию, деталь рук, предмет окружения, действие, смену крупности или атмосферу. Чередуй говорящего героя, конкретные предметные детали и атмосферные сцены по теме реплики; меняй крупность, угол камеры, жесты и положение рук, сохраняя одного героя и один outfit. Не создавай пустую панель одного помещения: если reference не даёт отдельного действия, оставь героя в кадре.",
