...
export function buildStoryboardImagePrompt(input: {
  ...
  return [
    ...
    productReferenceUrls.length
      ? "Поле продукта в каждом кадре определяет его роль: кадр с физически видимым продуктом показывает реальный продукт, а смысловой кадр посвящен предметам и окружению текущей реплики."
      : "",
    productPhysicalHint || "",
