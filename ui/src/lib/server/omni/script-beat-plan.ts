...
export function renderScriptBeatCue(
  ...
  return [
    ...
    ...lines,
    useAvatarWardrobe
      ? "Эти биты описывают только картинку, жесты и простые склейки. Речь брать только из строки ТОЧНАЯ РЕПЛИКА. Если внутри подсказки упомянута одежда reference-видео, игнорируй её и сохраняй outfit аватара. Не добавляй субтитры или текст на экран."
      : "Эти биты описывают только картинку, жесты и простые склейки. Речь брать только из строки ТОЧНАЯ РЕПЛИКА. Не добавляй субтитры или текст на экран.",
  ].join("\n");
}
