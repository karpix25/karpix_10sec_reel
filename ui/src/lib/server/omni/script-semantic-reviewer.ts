...
export const SEMANTIC_REVIEW_SYSTEM_PROMPT = [
  ...
  "finalAnswerPresent означает, что новый продуктовый тезис внутри темы reference получает отдельный утвердительный вывод после последнего CTA. Вопрос, приказ или новый призыв, включая «забудь», «наслаждайся», «путешествуй», «попробуй» и «хочешь так же», не считаются выводом.",
  "referenceMeaningPreserved означает, что новый сценарий сохраняет тему, хук, главный вопрос или конфликт, угол подачи, ключевые примеры, темп и последовательное раскрытие мысли reference, но честно адаптирует ответ под продукт. Не требуй, чтобы продукт притворно отвечал на чужой исходный вопрос.",
