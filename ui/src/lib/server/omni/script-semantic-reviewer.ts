...
const PRODUCT_CAPABILITY_RULES = [
  ...
  { label: "снятие или выдачу наличных", script: /банкомат|снят\w*\s+налич|выдач\w*\s+налич/iu, source: /банкомат|снят\w*\s+налич|выдач\w*\s+налич/iu },
  {
    ...
    script: /(?:с\s+ней|с\s+(?:этой\s+)?картой|плати\s+по\s+миру|виртуальн\w*\s+карт\w*)[^.!?]{0,160}(?:как\s+местн|цен\w*[^.!?]{0,30}(?:не\s+буд\w*\s+завыш|ниже)|не\s+переплач|скидк|выгодн\w*\s+курс)/iu,
...
export const SEMANTIC_REVIEW_SYSTEM_PROMPT = [
  ...
  "referenceMeaningPreserved означает сохранение тезиса, причинной связи, доказательства или примера и финального вывода исходного reference без подмены смысла. Если reference перечисляет конкретные места, действия, услуги или доказательства, в сценарии должны остаться минимум два конкретных примера и цена-payoff, если она была частью исходного предложения; общие слова вроде «виды» и «места» этого не заменяют.",
  "Не требуй сохранить все примеры длинного reference: двух конкретных примеров достаточно. Не отмечай остальные примеры как пропущенные.",
  "Каждый отдельный предмет, место, действие или услуга считается своим примером, даже если несколько перечислены в одном предложении через «и». Если сохранены минимум два таких элемента и цена-payoff, нельзя требовать заменить их другими примерами из reference.",
  ...
  "productNaturallyIntegrated означает, что продукт дан короткой рекламной вставкой в середине сценария: после раскрытого начала reference и до возврата к его сюжету. Вставка не обязана решать исходную проблему, и сам рекламный переход не является ошибкой.",
  "Рекламная вставка должна приписывать продукту только подтвержденную пользу. Нельзя утверждать, что карта предотвращает штраф, отменяет закон, выдает визу, гарантирует скидку или особый курс. Допустимо сказать, что карта помогает оплачивать покупки и услуги за границей.",
