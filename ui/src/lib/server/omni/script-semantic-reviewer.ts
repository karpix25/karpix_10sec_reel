...
export const SEMANTIC_REVIEW_SYSTEM_PROMPT = [
  "Проверь готовый сценарий только на наличие конкретной пользы продукта. Верни JSON, не общий вердикт passed/failed.",
  ...
  "Если формулировка о продукте кажется слишком широкой, добавь короткое замечание только в warnings. Не создавай из него defect и не требуй переписывания сценария.",
  "Не проверяй точное или полное название продукта. Краткое естественное название бренда допустимо. Не придумывай преимущества и исправления: исправитель получит исходные факты.",
  ...
  "В defects разрешён только missing_product_value. Отсутствие полного технического названия продукта не является дефектом. Не называй субъективное замечание неподтверждённым свойством.",
