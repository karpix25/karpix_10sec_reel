...
export const SEMANTIC_REVIEW_SYSTEM_PROMPT = [
  ...
  "productNaturallyIntegrated означает причинную и нативную интеграцию продукта в текущую мысль. В сценарии должна быть конкретная потребность, выбор или проблема из reference, на которую продукт отвечает как инструмент, пример или решение. Перед продуктом должен быть понятный причинный мостик, а польза должна быть объяснена именно для этой ситуации. Если предложение о продукте можно удалить без потери логики или пользы, проверка false. Отдельная рекламная вставка, внезапное упоминание и обязательное размещение в середине считаются ошибкой.",
...
function buildReviewPrompt(input: ScriptSemanticReviewInput) {
  ...
  return [
    ...
    "В evidence.value приведи короткую цитату, где объясняется польза продукта. В evidence.transition приведи короткую цитату или опиши причинный мостик от проблемы, выбора или потребности reference к продукту. Если мостика нет, оставь пустую строку. В evidence.answer приведи финальный ответ на главный вопрос или тезис.",
