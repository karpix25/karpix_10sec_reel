...
export const SEMANTIC_REVIEW_SYSTEM_PROMPT = [
  ...
  "productNaturallyIntegrated означает соблюдение единого правила интеграции ниже. Перед false проверь связь по всему готовому сценарию, включая потребность в хуке. В issues укажи, какая связь отсутствует или какое обещание не подтверждено.",
  ...
  "В repairInstructions дай конкретное исправление для каждой проваленной проверки: перехода к продукту, его названия или пользы, ответа на хук, сохранения смысла либо финала. Не ограничивай исправление перехода заменой заключительной фразы.",
  "Только при исправлении finalAnswerPresent предлагай декларативный финальный вывод. Не предлагай вопрос, приказ, CTA или императив вроде «не упустите», «посетите», «попробуйте» вместо этого вывода.",
...
function buildReviewPrompt(input: ScriptSemanticReviewInput) {
  return [
    ...
    "В evidence.value приведи короткую цитату, где объясняется польза продукта. В evidence.transition приведи точные цитаты из готового сценария, которые связывают потребность и применение продукта. Потребность может быть в хуке, а применение дальше по тексту. Не заменяй цитаты собственным объяснением придуманной связи. Если перехода в тексте нет, оставь пустую строку. В evidence.answer приведи финальный продуктовый вывод.",
