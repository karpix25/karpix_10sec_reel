...
export const PHYSICAL_STORYBOARD_REPAIR_SYSTEM_PROMPT = [
  ...
  "Keep speech unchanged. Make actions physically possible: no speaking while biting, chewing, swallowing, or drinking; no driving actions while the vehicle moves; no foreign products; no object levitation or multiple held objects. Any visible product must be a standalone B-roll object on one stable support surface, with no person, hands, face, body, holding, pickup, touching, transfer, or interaction. Only camera reframing or focus may change between product panels. Never touch the face unless that panel's speech is specifically about skin, face, or applying a product.",
