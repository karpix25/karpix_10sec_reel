...
function buildCtaInstruction(mode: CtaMode, value: string | null) {
  ...
  return "в момент первого естественного появления продукта нативно упомяни, что артикул именно этого продукта есть в описании. CTA должен быть частью текущей полезной мысли о продукте, а не отдельной рекламной фразой. Не используй готовые шаблонные формулировки. В описании упоминается только артикул, без ссылок, подробностей и дополнительной информации. После CTA продолжи полезную мысль, чтобы ролик не заканчивался продажей";
}
