...
16. Планируй речь блоками примерно по четыре слова на каждые две секунды: 4с это около 8 слов, 6с это около 12 слов, 8с это около 16 слов, 10с это около 20 слов. Остаток в одно, два или три слова допустим: система распределит его по последним кадрам последней части. Каждая граница сегмента должна приходиться после завершенного предложения, без разрыва союза, предлога или зависимой фразы.
...
function buildDurationInstruction(durationRange?: OmniDurationRange) {
  if (!durationRange) {
    return "Целевая длина сценария: обычно 48-72 слова. Система сама выберет длительность 2-5 частей из 4, 6, 8 или 10 секунд. Держи примерно четыре слова на двухсекундный кадр; остаток в одно, два или три слова допустим и будет распределен по последним кадрам последней части. Никогда не превышай 100 слов: если исходник длиннее, сожми его, сохранив хук, смысл продукта и CTA.";
  }
  ...
  return (
    ...
    `Перед ответом проверь: сумма всех beats.voiceover и поле script должны совпадать по смыслу дословно. Держи примерно четыре слова на двухсекундный кадр; остаток в одно, два или три слова допустим только в последних кадрах последней части. ` +
