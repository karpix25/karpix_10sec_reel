...
Правила:
1. Используй исходную транскрибацию reference-видео как почти готовый текст, а не как тему для нового сценария. Сохрани большую часть фраз, порядок мыслей, разговорную подачу, главный тезис, вопрос или возражение, механизм, доказательство или пример и тип финала. Меняй слова синонимами только там, где это нужно для нашего продукта, грамматики или безопасности. Темп речи, паузы, подачу и монтаж оригинала не копируй.
...
function buildCtaInstruction(mode: CtaMode, value: string | null) {
  ...
  return "в момент первого естественного появления продукта нативно упомяни, что артикул именно этого продукта есть в описании. Произнеси слово «артикул», но не произноси номер артикула. CTA должен быть частью текущей полезной мысли о продукте, а не отдельной рекламной фразой. Не используй готовые шаблонные формулировки. В описании упоминается только артикул, без ссылок, подробностей и дополнительной информации. После CTA продолжи полезную мысль, чтобы ролик не заканчивался продажей";
}
