...
Правила:
1. Используй исходную транскрибацию reference-видео вместе с контрактом адаптации ниже. Сохраняй только те смысловые опоры, которые обязательны для выбранного режима. Не пытайся сохранить большую часть фраз дословно. Если сохранение фраз конфликтует с лимитом длины, приоритет имеют лимит и контракт адаптации. Сделай минимальную редактуру внутри выбранного режима.
1а. ${renderScriptAdaptationContract(adaptationPlan)}
1а. Перед написанием сделай внутреннюю карту reference: один главный тезис, вопрос или возражение, механизм или объяснение "почему это работает", конкретные доказательства или примеры, порядок смысловых шагов и финальный вывод. Не показывай эту карту в ответе, но проверь по ней каждый beat. В режиме сохранения смысла сохрани каждую важную смысловую опору оригинала; в остальных режимах соблюдай контракт адаптации.
...
3. Переписывай reference в границах выбранного режима. В режиме «перенос формата» разрешено заменить исходную предметную тему новым продуктовым сюжетом; в остальных режимах сохраняй повестку и порядок обязательных мыслей. Чужой продукт не копируй.
