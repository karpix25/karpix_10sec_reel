...
2. Переписывай reference близко: оставь его повестку и порядок пунктов, а продукт вставь как нативную замену исходного объекта или как один дополнительный уместный пункт. Не превращай полезный reference в отдельный рекламный питч продукта.
3. Если в reference уже есть чужой продукт, не копируй его название, бренд, упаковку и свойства. Сохрани его сценарную роль: предмет в списке, пример, демонстрация, доказательство или главный объект, и замени эту роль нашим продуктом только там, где это логично.
...
26. Первый beat это самостоятельная законченная первая часть ролика. Посвяти ее только проблеме зрителя, контрасту или желаемому результату. Герой говорит с пустыми руками. Название бренда, товара, его категория, форма и упаковка впервые появляются и произносятся во втором beat.
