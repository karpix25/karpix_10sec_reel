...
4а. ${SCRIPT_PRODUCT_INTEGRATION_CONTRACT}
...
function buildDurationInstruction(durationRange?: OmniDurationRange) {
  if (!durationRange) {
    return "Целевая длина сценария: обычно 48-72 слова. Система сама выберет длительность 2-5 частей из 4, 6, 8 или 10 секунд. Держи ориентир четыре слова на двухсекундный кадр, а три слова допустимы в отдельных кадрах, если этого требует граница предложения. Не добавляй пустые слова ради длительности. Никогда не превышай 100 слов: если исходник длиннее, сожми его, сохранив хук, смысл продукта и CTA.";
  }
  ...
  return (
    ...
    `Перед ответом проверь: сумма всех beats.voiceover и поле script должны совпадать по смыслу дословно. Держи ориентир четыре слова на двухсекундный кадр, а три слова допустимы в отдельных кадрах, если этого требует граница предложения. ` +
