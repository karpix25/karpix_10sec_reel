...
8. В текстовых значениях JSON не используй дефисы, тире и минусы: -, —, –, ‒, ―, −. Если нужен разделитель, ставь запятую или точку. Также не используй слова "является", "в современном мире", "стоит отметить", "важно понимать".
...
19. Для каждого visual_cue сначала пойми смысл текущего voiceover, затем покажи именно этот смысл через персонажа, предметы или окружение. Продукт показывай только когда текущая реплика говорит о самом продукте, его свойствах или применении. Остальные темы показывай самостоятельной тематической сценой без продукта. Не копируй из reference несвязанные с текущей репликой процессы и предметы.
20. Поле script должно совпадать с beats.voiceover, склеенными по порядку.
21. Пиши без канцелярита и грамматических склеек. Нельзя: "продукт поддержать", "в идеале выбор зависит", повторять одно описательное слово продукта три раза.
22. Перед финальным JSON проверь все текстовые значения: нет emoji, нет дефисов, нет тире, нет минусов, нет цифр.
23. Поле background_audio_mood выбери строго из списка: energetic, calm, dramatic, inspiring, playful, serious.
24. Первый beat это самостоятельная законченная первая часть ролика. Посвяти ее только проблеме зрителя, контрасту или желаемому результату. Герой говорит с пустыми руками. Название бренда, товара, его категория, форма и упаковка впервые появляются и произносятся во втором beat.
25. Используй короткие законченные предложения. Вопросительный и восклицательный знаки ставь только там, где они естественно задают интонацию.
26. В visual_cue описывай только то, что физически находится и происходит внутри кадра: сцену, композицию, камеру и простое действие.
...
Верни JSON строго такого вида:
{
  ...
  "beats": [
    {
      "stage": "hook",
      "visual_cue": "${visualCueExample}; герой с пустыми руками",
      "voiceover": "законченный хук только о проблеме или желаемом результате"
    },
    {
      "stage": "body",
...
function buildCtaInstruction(mode: CtaMode, value: string | null) {
  ...
  return "в финале коротко скажи: «Артикул в описании.» Если номера нет в данных, не выдумывай его";
}
