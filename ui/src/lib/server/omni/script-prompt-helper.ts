...
export function buildPrompt(input: {
  ...
  return `
Адаптируй исходный сценарий reference-видео для Instagram Reels по методологии сценариста Reels.

Правила:
1. Используй исходную транскрибацию reference-видео как почти готовый текст, а не как тему для нового сценария. Reference это исходник: сохрани его хук, порядок мыслей, разговорную подачу, главный тезис, вопрос или возражение, механизм, доказательство или пример и тип финала. Не придумывай новую тему, новый угол или новый сюжет. Меняй только то, что нужно для нашего продукта, грамматики, безопасности и длины. Если фраза помещается, сохраняй близкую формулировку. Если весь текст не помещается, действуй так: Не пытайся сохранить большую часть фраз дословно, сначала сохраняй хук и смысловые опоры, а сокращай повторы и лишние слова. Меняй слова синонимами только там, где это нужно для нашего продукта, грамматики или безопасности. Не добавляй новые рекламные аргументы и не перестраивай повестку. Темп речи, паузы, подачу и монтаж оригинала не копируй.
...
1б. Если нужно сократить текст до четырех частей, сначала убирай повторы, вводные слова и лишние пояснения, затем сокращай длинные формулировки. Хук reference и порядок смысловых шагов сохраняй. Объединяй близкие предложения в один плотный beat. Не заменяй конкретный механизм, пример, доказательство или вывод общей фразой вроде "это полезно" или "помогает лучше себя чувствовать".
...
7. Структура: сначала повтори механику и хук reference, затем сохрани его порядок смысловых шагов, один CTA в том же сценарном месте. Итоговый сценарий должен укладываться в 3-4 части, не создавай пятую часть и не добавляй новую смысловую линию.
...
19. Сделай 3 близких варианта hook_options на основе хука reference, а не 3 новых идеи. selected_hook должен быть самым близким к исходному хуку вариантом после адаптации под наш продукт.
