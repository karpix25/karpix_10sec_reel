...
function buildStoryboardImagePrompt(input: {
  ...
  return [
    ...
    "Каждый кадр должен быть отдельной визуальной панелью с маленьким номером кадра.",
    "В панелях только реальная сцена: человек, продукт, фон, руки и предметы. Не добавляй декоративную графику, экранный текст, стрелки, подписи и SFX-плашки.",
    ...
    ...input.storyboard.frames.map((frame, index) =>
      [
        ...
        "картинка чистая, без декоративной графики.",
      ].join(" ")
    ),
  ].join("\n");
}
