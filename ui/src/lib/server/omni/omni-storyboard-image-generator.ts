...
function buildStoryboardImagePrompt(input: {
  ...
  return [
    ...
    "Стандарт макета обязателен: черный фон, ровно 5 вертикальных карточек-кадров в один горизонтальный ряд, тонкие белые разделители, без второго ряда и без таблиц.",
    "Каждая карточка состоит из большого визуального кадра сверху и небольшой черной зоны подсказок снизу.",
    "В левом верхнем углу каждой карточки нарисуй белый круг с номером кадра 1, 2, 3, 4, 5.",
    "То, что нужно озвучить, напиши прямо поверх визуального кадра крупной белой рукописной или жирной Reels-style надписью на русском языке.",
    "В нижней черной зоне каждой карточки добавь только маленькие серые подсказки для модели: РАКУРС: ...; ЭФФЕКТ: ...; SFX: ... .",
    ...
    previousStoryboardReferenceUrl
      ? `Изображение ${previousReferenceIndex} - предыдущая раскадровка этого же ролика. Используй ее как continuity reference: тот же стиль, персонаж, одежда, свет, окружение, масштаб продукта и тот же макет из 5 вертикальных карточек. Не копируй старые действия и текст.`
      : "",
    "Слова озвучки должны появиться на картинке только один раз: прямо на визуале соответствующего кадра. Не повторяй эти фразы в подсказках.",
    "Каждый кадр должен быть отдельной вертикальной визуальной панелью с таймингом 2 секунды; текст кадра 3-4 слова, крупно и читаемо.",
    ...
    ...input.storyboard.frames.map((frame, index) =>
      [
        ...
        "нижняя подсказка должна быть короткой: РАКУРС, ЭФФЕКТ, SFX.",
      ].join(" ")
    ),
  ].join("\n");
}
