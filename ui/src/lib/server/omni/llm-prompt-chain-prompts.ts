...
function buildCtaLine(mode: CtaMode, value: string | null) {
  ...
  return "в момент первого естественного появления продукта объяснить, как найти именно этот вариант через артикул в описании. CTA должен быть частью мысли о выборе продукта, например «чтобы не перепутать с похожими, артикул будет в описании» или «если будете искать именно этот вариант, артикул будет в описании». В описании упоминается только артикул, без ссылок, подробностей и дополнительной информации. Не ставь CTA отдельной финальной фразой";
}
