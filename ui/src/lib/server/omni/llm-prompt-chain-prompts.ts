...
Первая часть сценария это самостоятельный законченный хук только о проблеме зрителя или желаемом результате. Название бренда, товара, его категория, форма и упаковка впервые произносятся во второй части.
...
Первый segment это самостоятельный talking head hook: герой с пустыми руками, товар вне кадра, все storyboard frames используют reference_role avatar или none, а средняя перебивка имеет роль environment_cutaway. Voiceover первого segment говорит только о проблеме зрителя или желаемом результате. Первое упоминание и первое появление бренда, товара, его категории, формы и упаковки начинается во втором segment.
...
Выбирай product_cutaway только когда смысл spoken_words этого frame прямо требует показать продукт, его свойства или применение. В остальных случаях используй environment_cutaway с тематическими предметами или окружением без продукта. В product_cutaway продукт обязан быть физически видимым и детально совпадать с product reference.
Одежда, свет, окружение и типаж героя должны быть едиными во всех frames одного ролика.
Добавляй динамику UGC внутри сцены: выразительный первый ракурс, живые жесты, смену крупности и простые смысловые микродействия без прямой рекламной подачи.
...
Первый segment показывает только героя с пустыми руками. Товар впервые появляется во втором segment.
...
Усиль динамику UGC внутри кадров: первый кадр с выразительным ракурсом или действием, живые жесты, смена крупности и одна конкретная смысловая сцена, но без рекламной постановки.
Все числа в текстовых значениях JSON пиши словами. Не используй emoji, дефисы, тире или минусы.
...
function buildCtaLine(mode: CtaMode, value: string | null) {
  ...
  return "в момент первого естественного появления продукта сказать, что главный продукт есть в описании, потом продолжить полезную мысль. Не ставь CTA отдельной финальной фразой";
}
