...
function buildDurationLine(durationRange?: OmniDurationRange) {
  ...
  return [
    ...
    "Сначала напиши естественный цельный voiceover. Затем планировщик сам распределит его по законченным предложениям в части 4/6/8/10 секунд, примерно по четыре слова на каждые две секунды. Общая длительность обязана попасть в заданный диапазон; если текст не помещается в верхнюю границу, в следующей попытке убери второстепенные детали, а не добавляй пустые фразы. Не делай отдельными предложениями фрагменты из одного-трех слов. Каждая граница segment должна приходиться после завершенного предложения, без разрыва союза, предлога или зависимой фразы.",
  ].join(" ");
}
