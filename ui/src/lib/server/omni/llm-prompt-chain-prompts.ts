...
Если reference не называет продукт в хуке, не вставляй его искусственно в первую фразу: назови его в момент, когда появляется практическое решение. К финалу зритель должен понимать, что это за продукт и зачем он нужен.
...
Не заменяй тему reference темой продукта. Встрой продукт через конкретную потребность, действие или ситуацию, которые уже есть в исходной истории.
Перед CTA произнеси полноценный финальный вывод, завершающий главный тезис reference. CTA может быть отдельной последней фразой, но не заменяет вывод.
...
function buildCtaLine(mode: CtaMode, value: string | null) {
  ...
  if (mode === "link_in_profile") return `в финальном voiceover обязательно произнеси точную фразу «ссылка в профиле»${value ? `, цель ссылки: ${value}` : ""}; не упоминай описание, комментарии или кодовые слова`;
