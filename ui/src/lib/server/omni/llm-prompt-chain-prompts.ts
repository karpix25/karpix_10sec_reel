...
Меняй слова синонимами только там, где это нужно для нашего продукта, грамматики или безопасности.
...
function buildCtaLine(mode: CtaMode, value: string | null) {
  ...
  return "в момент первого естественного появления продукта нативно упомянуть, что артикул именно этого продукта есть в описании. Произнести слово «артикул», но не произносить номер артикула. CTA должен быть частью текущей полезной мысли о продукте, а не отдельной рекламной фразой. Не используй готовые шаблонные формулировки. В описании упоминается только артикул, без ссылок, подробностей и дополнительной информации. Не ставь CTA отдельной финальной фразой";
}
