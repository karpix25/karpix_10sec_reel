...
export function buildCreativeCopywriterPrompt(input: PromptChainInput) {
  ...
  const referenceRules = isFormatTransfer
    ? [
      "Reference передаёт форму хука, личную подачу, темп, структуру и тип раскрытия. Полностью замени исходный предмет новым честным сюжетом о продукте; не сохраняй чужие факты, механизм, место, результат или ответ.",
      "Перед написанием определи внутренне, чем цепляет форма хука, как устроено раскрытие и где появляется payoff. Не пытайся отвечать на исходный предметный вопрос и не переноси его обязательные пункты.",
    ].join("\n")
    : [
      "Reference передаёт не только хук, темп и структуру, но и тему, главный вопрос или конфликт, угол подачи, смысловую логику, аргументы и конкретные примеры. Возьми reference целиком и напиши новый сценарий на ту же тему, но адаптируй его под наш продукт. Не копируй текст дословно.",
      "Перед написанием определи внутренне: чем цепляет хук, какую тему и проблему раскрывает reference, какие примеры поддерживают мысль и в какой момент зрителю становится понятен ответ. Не показывай этот разбор в ответе.",
    ].join("\n");
  const firstPartRule = isFormatTransfer
    ? "Первая часть сценария сохраняет форму хука и механику подачи reference без нашего продукта."
    : "Первая часть сценария повторяет хук и механику reference без нашего продукта.";
  const hookAnswerRule = isFormatTransfer
    ? "Не отвечай на исходный предметный вопрос. Новый хук и его ответ должны раскрывать продуктовую проблему; упоминание продукта и CTA не считаются ответом на новый хук."
    : "Сначала раскрой обещание или вопрос хука и дай зрителю конкретный ответ в рамках темы reference. Упоминание продукта и CTA не считаются ответом на хук.";
  const bridgeRule = isFormatTransfer
    ? "Полностью замени исходную тему на реальную потребность, которую продукт решает. Не притворяйся, что продукт решает чужую проблему."
    : "Если продукт не отвечает на исходный вопрос напрямую, сохрани исходную тему как контекст и добавь естественный переход к реальной потребности, которую продукт решает. Не притворяйся, что продукт решает чужую проблему.";
  const conclusionRule = isFormatTransfer
    ? "Финальный вывод должен завершать новый продуктовый тезис, а не возвращать зрителя к исходному предмету reference."
    : "Финальный вывод должен завершать продуктовый тезис внутри темы reference, а не возвращать зрителя к нерешённому исходному вопросу и не подменять пользу продукта общей рекламой.";
  const hookFactRule = isFormatTransfer
    ? "Если форма хука обещает конкретный факт или результат, дай новый подтвержденный ответ о продукте, не переноси обещание reference."
  ...
  const listRule = isFormatTransfer
    ? "Не переноси предметные списки и обязательные пункты reference, если они относятся к замененному исходному механизму."
    : "Если в reference есть список, сохрани его тему, форму и количество пунктов, если это совместимо с продуктом. Не заменяй пункты рекламной вставкой.";
  const compressionRule = isFormatTransfer
    ? "Если нужно сократить текст, убирай повторы и вводные слова, а не название продукта, его пользу, CTA или финальный вывод."
...
function buildDurationLine(durationRange?: OmniDurationRange) {
  ...
  return [
    ...
    "Стремись к этой длине, но не урезай смысл ради верхней границы: если текст естественно длиннее, сохрани его до общего лимита 100 слов, а раскадровка добавит часть. Используй только количество слов, кратное четырем, и не создавай больше пяти частей.",
  ].join(" ");
}
