...
КОМПАКТНОСТЬ JSON
- Верни JSON без markdown, комментариев, отступов и повторения одних данных в разных полях.
- Все служебные описания делай краткими: одна конкретная фраза, обычно не больше двенадцати слов.
- В каждом timeline используй не больше трёх интервалов, если в reference действительно не происходят заметные изменения.
- format_grammar.beat_sequence содержит только обязательные уникальные операции, максимум шесть элементов.
- Не раздувай visual_description, action, camera, sfx и notes синонимами или художественными пояснениями.
- Полными остаются только spoken_transcript, total_voiceover, segment.voiceover и storyboard spoken_words.
