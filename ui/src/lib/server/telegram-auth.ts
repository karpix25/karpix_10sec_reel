...
export async function ensureTelegramAuthTables() {
  ...
  // If user access is already approved/admin, pending web-auth requests are stale and should not
  // appear in admin approval queues.
  await pool.query(
    `UPDATE telegram_web_auth_requests r
     ...
     WHERE r.status = 'pending'
       ...
       AND EXISTS (
         SELECT 1
         FROM telegram_user_access a
         WHERE a.telegram_user_id = r.telegram_user_id
           AND (a.status = 'approved' OR a.is_admin = TRUE)
       )`
  );
}
