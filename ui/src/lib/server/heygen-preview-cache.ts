...
const CACHE_FAILURE_COOLDOWN_MS = 60_000;
...
function isPreviewCacheDisabled(): boolean {
  return Date.now() < cacheDisabledUntil;
}

function handlePreviewCacheDbError(operation: string, error: unknown): void {
  ...
  console.warn(
    `HeyGen preview cache ${operation} unavailable, bypassing DB cache for ${Math.round(CACHE_FAILURE_COOLDOWN_MS / 1000)}s: ${message}`
  );
}
...
export async function getStableHeygenPreviewUrl(params: {
  ...
  try {
    ...
    handlePreviewCacheDbError("read", error);
    return sourceUrl;
  }
  ...
  if (sourceUrl && isHttpUrl(sourceUrl)) {
    try {
      ...
      handlePreviewCacheDbError("write", error);
    }
  }
...
export async function getCachedHeygenPreviewBlob(cacheKeyInput: unknown): Promise<CachedPreviewBlob | null> {
  ...
  try {
    ...
    handlePreviewCacheDbError("blob read", error);
    return null;
  }
