...
import { linkLegacyLibrary, listLegacyLibraryLinks, unlinkLegacyLibrary } from "@/lib/server/omni/legacy-library-links";
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await unlinkLegacyLibrary({ projectId, productId, legacyClientId }));
  } catch (error) {
    console.error("Omni legacy library unlink error:", error);
    return jsonError(error instanceof Error ? error.message : "Internal Server Error", 500);
  }
}
