...
import pool, { oldPool } from '@/lib/db';
...
export async function GET(request: Request) {
  ...
  try {
    const libRes = await pool.query(
      'SELECT library_id FROM client_libraries WHERE client_id = $1',
      [clientId]
    );
    const libIds = libRes.rows.map(r => r.library_id);
