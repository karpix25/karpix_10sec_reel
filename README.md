...
6. После completion каждого segment app скачивает видео, вытаскивает последний кадр через FFmpeg и сохраняет его как continuity frame.
7. Следующий segment отправляется только после completion предыдущего и получает его last frame как reference image. Для KIE этот кадр сначала загружается через KIE File Upload API, а avatar по-прежнему передается только через `character_ids`.
8. Готовые segment videos склеиваются через FFmpeg и загружаются в configured storage.
