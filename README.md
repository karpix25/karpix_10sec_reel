...
3. Если draft-сценарий пишется по конкретному legacy original, app on-demand получает прямой MP4 через ScrapeCreators, копирует reference video в S3 при доступной конфигурации и сохраняет OpenRouter/MiniMax director brief в `omni_legacy_video_analyses`.
4. `ui/src/lib/server/omni/reels.ts` создает `omni_reels` и планирует `omni_reel_segments`.
5. `ui/src/lib/server/omni/omni-prompt-builder.ts` рендерит по одному prompt на segment: spoken text, product context, character contract, clothing contract, CTA rules, selected visual format и сохраненную режиссуру оригинала, если она есть.
6. `ui/src/lib/server/omni/omni-reel-runner.ts` отправляет segments выбранному провайдеру:
   - CometAPI использует `omni-fast`.
   - KIE использует `gemini-omni-video` и требует сохраненный `character_id`.
7. После completion каждого segment app скачивает видео, вытаскивает последний кадр через FFmpeg и сохраняет его как continuity frame.
8. Следующий segment отправляется только после completion предыдущего и получает его last frame как reference image. Для KIE этот кадр сначала загружается через KIE File Upload API, а avatar по-прежнему передается только через `character_ids`.
9. Готовые segment videos склеиваются через FFmpeg и загружаются в configured storage.
...
| --- | --- | --- |
| OpenRouter | Script, visual, prompt generation и director analysis через `google/gemini-3.5-flash-lite` | `OPENROUTER_API_KEY`, `SCENARIO_MODEL`, `OMNI_DIRECTOR_ANALYSIS_MODEL` |
...
Omni continuity chain включен по умолчанию. Для аварийного отключения sequential last-frame handoff используйте `OMNI_CONTINUITY_CHAIN=false`.
...
node scripts/test_omni_director_analysis_contract.mjs
```
