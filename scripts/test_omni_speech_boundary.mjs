...
try {
  ...
  const latestCollagenSegments = splitScriptIntoVoiceSegments(
    "Есть ли смысл принимать коллаген после тридцати пяти лет? Конечно есть. Потому что коллаген это аминокислоты. А из аминокислот как раз и строятся наши гормоны. Из белка строится наша кожа, строятся абсолютно все гормоны и рецепторы. Если вам интересно, подробнее про этот коллаген, артикул есть в описании. А у нас белка часто не хватает в рационе. Поэтому принимать его имеет смысл в любом возрасте.",
    3,
    24
  );
  assert.ok(
    !latestCollagenSegments.some((segment) => /подробнее\s+про\s+этот$/iu.test(segment.text)),
    "latest collagen script must keep the demonstrative phrase with its noun"
  );
  assert.equal(
    reconstructVoiceSegments(latestCollagenSegments),
    "Есть ли смысл принимать коллаген после тридцати пяти лет? Конечно есть. Потому что коллаген это аминокислоты. А из аминокислот как раз и строятся наши гормоны. Из белка строится наша кожа, строятся абсолютно все гормоны и рецепторы. Если вам интересно, подробнее про этот коллаген, артикул есть в описании. А у нас белка часто не хватает в рационе. Поэтому принимать его имеет смысл в любом возрасте."
  );
