...
try {
  ...
  assert.equal(payload.messages[1].content.find((item) => item.type === "video_url").video_url.url,
    "https://example.com/reference.mp4", "paid client must use the direct Scrape Creators video URL");
  ...
  await analyzeDirectorVideo({ videoUrl: "https://example.com/large.mp4", transcript: "" });
  assert.equal(modelCalls, 2, "a direct video URL must be sent to the model without local size preflight");
