...
function buildInput(wardrobeSource) {
  const voiceSegments = [
    "Этот крем быстро вписывается в утренний уход и не перегружает кожу каждый день даже при активном графике.",
    "Текстура легкая и не оставляет жирного блеска поэтому кожа выглядит свежей весь день в любом ритме.",
    "Артикул можно найти в описании если хотите спокойно сравнить характеристики и заказать крем без лишних сомнений.",
  ];
  return {
    generatedScript: {
      id: 88,
