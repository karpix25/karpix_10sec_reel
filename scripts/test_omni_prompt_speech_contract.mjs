...
try {
  ...
  for (const item of prompts) {
    ...
    assert.ok(item.prompt.includes("персонаж в кадре сам произносит"), "storyboard prompt must require character speech");
    assert.ok(item.prompt.includes("это не закадровый голос"), "storyboard prompt must forbid background narration");
    ...
    assert.ok(item.prompt.length < 1400, "storyboard prompt must stay short");
